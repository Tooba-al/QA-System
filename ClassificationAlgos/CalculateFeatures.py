...
def get_edit_distance(question, span):
    question = stopword_func(question)
    span = stopword_func(span)
    ...
    ED = 0
    if ED_span > ED_question:
        ED = ED_span - ED_question
    else:
        ED = ED_question - ED_span

    return ED


def get_question_types(question):
    wh_words = [
        "which",
        "where",
        "what",
...
def get_features(question, span, answer, titleNo, paragNo):
    wh_word = get_question_types(question)
    question_length = get_length(question)

    if span == "-":
        syntactic_divergence = "-"
        ...
        syntactic_divergence = get_syntatic_div(question, span, answer)
        ...
        edit_distance = get_edit_distance(question, span)
...
def calculateFeatures_main(CSV_file):
    # dict_datas = []
    ...
    spans = CSV["span"].tolist()
    answers = CSV["answer"].tolist()
    ...
    with open(csv_file, "w") as features_file:
        ...
        for index in range(744, len(questions)):
            ...
            # sentences = nltk.sent_tokenize(contexts[index])
            # for sentence in sentences:
            # if sentence.count(answers[index]) > 0:
            this_result = get_features(
                questions[index],
                spans[index],
                answers[index],
                titleNos[index],
    ...
    # csv_file = "ClassificationAlgos/Features_dev1_p52_1.csv"
    # csv_columns = [
    #     "paragNo",
    #     "titleNo",
    #     "question",
...
end_time = time.time()
print(Fore.YELLOW + "Execution Time = ", str((end_time - start_time) / 60) + "min")
